...
class KoreaInvestmentWS(Process):
    ...
    def run(self):
        """_summary_"""
        asyncio.run(self.ws_client(), debug=True)
    ...
    def get_approval(self) -> str:
        ...
        body = {"grant_type": "client_credentials", "appkey": self.api_key, "secretkey": self.api_secret}
...
class KoreaInvestment:
    ...
    def __init__(
        self, api_key: str, api_secret: str, acc_no: str, user_id: str, exchange: str = "서울", mock: bool = False
    ):
        """생성자
        Args:
    ...
    def issue_access_token(self):
        ...
        url = f"{self.base_url}/{path}"
        headers = {"content-type": "application/json"}
        data = {"grant_type": "client_credentials", "appkey": self.api_key, "appsecret": self.api_secret}
    ...
    def fetch_today_1m_ohlcv(self, symbol: str, to: str = ""):
        ...
        while last_hour > "090100":
            ...
            dt1 = datetime.datetime(
                year=now.year, month=now.month, day=now.day, hour=int(last_hour[:2]), minute=int(last_hour[2:4])
            )
    ...
    def fetch_ohlcv(
        self, symbol: str, timeframe: str = "D", start_day: str = "", end_day: str = "", adj_price: bool = True
    ) -> dict:
    ...
    def cancel_order(
        self, org_no: str, order_no: str, quantity: int, total: bool, order_type: str = "00", price: int = 100
    ):
        """주문 취소
    ...
    def modify_order(self, org_no: str, order_no: str, order_type: str, price: int, quantity: int, total: bool):
        """주문정정
        ...
        Returns:
            dict : _description_
        """
        return self.update_order(org_no, order_no, order_type, price, quantity, True, total)
    ...
    ## TODO: 날짜 3개월 넘어갈때 어떻게 할지 결정하여 수정
    def fetch_trade_list(self, ticker):
        ...
        since = (datetime.datetime.today() - datetime.timedelta(weeks=24)).strftime("%Y%m%d")
        params = {
            "CANO": self.acc_no_prefix,
            "ACNT_PRDT_CD": self.acc_no_postfix,
            "INQR_STRT_DT": since,
    ...
    def fetch_search_condtion_list(self):
        """종목조건검색 목록조회
        주의사항 : RSI는 계산방식이 다르므로 사용하지 않는 것을 권장
        Args:
...
if __name__ == "__main__":
    ...
    broker = KoreaInvestment(api_key=key, api_secret=secret, acc_no=acc_no, exchange="나스닥")
